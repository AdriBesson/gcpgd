...
class CHSDAlgorithm(BaseReconstructionAlgorithm):
    """
    Class for the CHSD algorithm, with parent class `BaseReconstructionAlgorithm`.
    ...
    def __init__(self, nb_iter: int, linear_op: LinearOperatorFromMatrix, toeplitz_op: ToeplitzificationOperator,
        ...
        Backend of Cazow denoising.
        """
        super(CHSDAlgorithm, self).__init__(nb_iter=nb_iter,
                                            nb_init=1, name='CHSD', random_state=random_state)
        ...
        if not isinstance(toeplitz_op, ToeplitzificationOperator):
            raise ValueError(
                "Argument toeplitz_op must be an instance of ToeplitzificationOperator class.")
    ...
    def initialize(self, y: np.ndarray) -> list:
        ...
        if self.provided_init_sol is not None:
            ...
            if self.nb_init == 1:
                init_sol = np.zeros(
                    shape=(self.linear_op.shape[1],), dtype=np.complex128)
            else:
                print('CHSD randomly initialized!')
    ...
    def init_tau(self, type: str = 'safest', weight: float = 1.5):
        ...
        if self.beta is not None:
            ...
            try:
                beta = eigs(weighted_gram, k=1, which='LM',
                            return_eigenvectors=False, tol=self.eig_tol)
                beta *= (1 + self.eig_tol)
        ...
        if type == 'fastest':
            try:
                alpha = eigs(weighted_gram, k=1, which='SM',
                             return_eigenvectors=False, tol=self.eig_tol)
    ...
    def iterate(self, x: list) -> list:
        ...
        x_sd = x[0] - tau * derivative
        z = self.denoising_algorithm.reconstruct(
            x[0], verbose=self.denoise_verbose, verbose_frequency=1)
    ...
    def stop_criterion(self, x: list) -> dict:
        ...
        if np.linalg.norm(self.x_old[0]) == 0:
            ...
            relative_improvement = np.linalg.norm(
                (x[0] - self.x_old[0])) / np.linalg.norm(self.x_old[0])
