...
# ############################################################################
"""
Class for the RED-CPGD algorithm. Description and analysis of the algorithm available at:
...
class CPGDREDSDAlgorithm(BaseReconstructionAlgorithm):
    """
    Class for the RED-CPGD steepest descent algorithm, with parent class `BaseReconstructionAlgorithm`.
    ...
    def __init__(self, nb_iter: int, linear_op: LinearOperatorFromMatrix, toeplitz_op: ToeplitzificationOperator,
                 rank: int, nb_cadzow_iter: int = 20, denoise_verbose: bool = False, mu :int = 1,
                 tol: float = 1e-6, eig_tol: float = 1e-8, init_sol: np.ndarray = None, tau : float = None, tau_init_type: str = 'safest', tau_weight: float = 1.5, lam : float = 1, nb_init : int = 1, random_state : int = 1,
                 cadzow_backend: str = 'scipy'):
        """
        ...
        Backend of Cazow denoising.
        """
        super(CPGDREDSDAlgorithm, self).__init__(nb_iter=nb_iter, nb_init=nb_init, name='CPGD-RED-SD', random_state=random_state)
    ...
    def initialize(self, y: np.ndarray) -> list:
        ...
        if self.provided_init_sol is not None:
            ...
            if self.nb_init == 1:
                ...
                print('CPGD-RED-SD randomly initialized!')
    ...
    def iterate(self, x: list) -> list:
        ...
        """
        y = x[1]
        grad = lambda x : 1 / self.tau * (self.linear_op.rmatvec(self.linear_op.matvec(x) - y) + self.lam * (x - self.denoising_algorithm.denoise(x))) 
        
        # steepest descent
        x[0] = x[0] - self.mu * grad(x[0])
        return x
