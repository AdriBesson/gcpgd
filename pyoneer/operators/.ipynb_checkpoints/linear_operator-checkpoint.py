...
class MyToeplitzificationOperator(AbstractLinearOperator):
    """
    ...
    def __init__(self, P: int, M: int, dtype: type = np.complex128):
        ...
        super(MyToeplitzificationOperator, self).__init__(shape=shape, dtype=dtype)
    ...
    def _matvec(self, x: np.ndarray):
        ...
        if x.size != self.N:
            ...
            x = np.diag(np.sqrt(1./self.gram)) @ x
    ...
    def _rmatvec(self, x: np.ndarray):
        ...
        if x.size != self.shape[0]:
            ...
            out = np.diag(np.sqrt(1./self.gram)) @ out
            return out
...
def build_toeplitz_operator(P: int, M: int, x: np.ndarray, toeplitz_class: str = 'standard', method=None) -> Union[
    ...
    """
    if toeplitz_class in ['standard', 'matrix_free', 'alternative']:
    ...
    elif chosen_toeplitz_class == 'alternative':
        Tp = MyToeplitzificationOperator(P=P, M=M, dtype=x.dtype)
        return LinearOperatorFromMatrix(mat=Tp.matvec(x))
    else:
        return ToeplitzMatrixFree(P, M, x, measure=False, method=method)


def choose_toeplitz_class(P: int, M: int, measure: bool = False, avg_size: int = 10, method='standard'):
    """
